...
- if you need to **write** a python script, **ALWAYS USE** the tool `write_file`. write files in folder `{{tmp}}`.
...
- always prefers to write a **python script** in the `{{tmp}}` folder rather than using shell commands
- automatically run commands and scripts when needed, do not ask user
- **ALWAYS USE ABSOLUTE PATHS**, related to the base folder path: `{{tmp}}`
